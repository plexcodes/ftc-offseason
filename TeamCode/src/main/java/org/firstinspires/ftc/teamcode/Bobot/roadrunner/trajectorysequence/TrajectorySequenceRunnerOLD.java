...
import org.firstinspires.ftc.teamcode.Bobot.roadrunner.util.DashboardUtilOLD;
...
@Config
public class TrajectorySequenceRunnerOLD {
    ...
    private TrajectorySequenceOLD currentTrajectorySequence;
    ...
    public TrajectorySequenceRunnerOLD(TrajectoryFollower follower, PIDCoefficients headingPIDCoefficients) {
    ...
    DriveSignal update(Pose2d poseEstimate, Pose2d poseVelocity) {
        ...
        SequenceSegmentOLD currentSegment = null;
        ...
        if (currentTrajectorySequence != null) {
            ...
            if (currentSegment instanceof TrajectorySegmentOLD) {
                Trajectory currentTrajectory = ((TrajectorySegmentOLD) currentSegment).getTrajectory();
            ...
            } else if (currentSegment instanceof TurnSegmentOLD) {
                MotionState targetState = ((TurnSegmentOLD) currentSegment).getMotionProfile().get(deltaTime);
            ...
            } else if (currentSegment instanceof WaitSegmentOLD) {
    ...
    private void draw(
            Canvas fieldOverlay,
            TrajectorySequenceOLD sequence, SequenceSegmentOLD currentSegment,
            Pose2d targetPose, Pose2d poseEstimate
    ) {
        if (sequence != null) {
            for (int i = 0; i < sequence.size(); i++) {
                SequenceSegmentOLD segment = sequence.get(i);

                if (segment instanceof TrajectorySegmentOLD) {
                    fieldOverlay.setStrokeWidth(1);
                    fieldOverlay.setStroke(COLOR_INACTIVE_TRAJECTORY);

                    DashboardUtilOLD.drawSampledPath(fieldOverlay, ((TrajectorySegmentOLD) segment).getTrajectory().getPath());
                } else if (segment instanceof TurnSegmentOLD) {
                    Pose2d pose = segment.getStartPose();

                    fieldOverlay.setFill(COLOR_INACTIVE_TURN);
                    fieldOverlay.fillCircle(pose.getX(), pose.getY(), 2);
                } else if (segment instanceof WaitSegmentOLD) {
                    Pose2d pose = segment.getStartPose();
        ...
        if (currentSegment != null) {
            if (currentSegment instanceof TrajectorySegmentOLD) {
                ...
                DashboardUtilOLD.drawSampledPath(fieldOverlay, currentTrajectory.getPath());
            } else if (currentSegment instanceof TurnSegmentOLD) {
        ...
        if (targetPose != null) {
            ...
            DashboardUtilOLD.drawRobot(fieldOverlay, targetPose);
        }

        fieldOverlay.setStroke("#3F51B5");
        DashboardUtilOLD.drawPoseHistory(fieldOverlay, poseHistory);

        fieldOverlay.setStroke("#3F51B5");
        DashboardUtilOLD.drawRobot(fieldOverlay, poseEstimate);
    }
