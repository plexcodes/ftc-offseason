...
public class BNO055IMUUtilOLD {
     ...
     */
    public static void swapThenFlipAxes(BNO055IMU imu, AxesOrder order, AxesSignsOLD signs) {
        try {
     ...
     * {@link AxisDirectionOLD}. See {@link #swapThenFlipAxes} for details about the remapping.
     *
     * @param imu IMU
     * @param direction axis direction
     */
    public static void remapZAxis(BNO055IMU imu, AxisDirectionOLD direction) {
        switch (direction) {
            case POS_X:
                swapThenFlipAxes(imu, AxesOrder.ZYX, AxesSignsOLD.NPP);
                break;
            case NEG_X:
                swapThenFlipAxes(imu, AxesOrder.ZYX, AxesSignsOLD.PPN);
                break;
            case POS_Y:
                swapThenFlipAxes(imu, AxesOrder.XZY, AxesSignsOLD.PNP);
                break;
            case NEG_Y:
                swapThenFlipAxes(imu, AxesOrder.XZY, AxesSignsOLD.PPN);
                break;
            case POS_Z:
                swapThenFlipAxes(imu, AxesOrder.XYZ, AxesSignsOLD.PPP);
                break;
            case NEG_Z:
                swapThenFlipAxes(imu, AxesOrder.XYZ, AxesSignsOLD.PNN);
                break;
        }
    }
     ...
     */
    @Deprecated
    public static void remapAxes(BNO055IMU imu, AxesOrder order, AxesSignsOLD signs) {
        AxesOrder adjustedAxesOrder = order.reverse();
        int[] indices = order.indices();
        int axisSignValue = signs.bVal ^ (0b100 >> indices[0]);
        AxesSignsOLD adjustedAxesSigns = AxesSignsOLD.fromBinaryValue(axisSignValue);
